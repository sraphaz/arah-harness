...
export function Nav({ locale }: { locale: Locale }) {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-arah-hair2 bg-[rgba(10,12,15,0.82)] backdrop-blur-[14px]">
      ...
      <nav
        className="mx-auto flex min-h-16 max-w-wide flex-wrap items-center gap-x-5 gap-y-2 px-8 py-3"
        aria-label="Primary"
      >
        <Link
          href={localePath(locale)}
          className="flex items-center gap-2.5 font-display text-[15px] font-semibold text-arah-text"
        >
          <Logo />
          <span>
            ARAH <span className="font-normal text-arah-muted">Harness</span>
          </span>
        </Link>
            ...
            return (
              <Link
                ...
                className={`font-mono text-[12.5px] tracking-wide ${
                  active
                    ? "text-arah-text"
                    : "text-arah-muted hover:text-arah-text"
                }`}
              >
                {l.label}
              </Link>
        ...
        <div className="flex items-center gap-2">
          <div
            className="flex overflow-hidden rounded-control border border-arah-control font-mono text-[11px]"
            role="group"
            aria-label="Language"
          >
            <Link
              href={swapLocalePath(pathname, "en")}
              className={`px-2.5 py-1.5 ${locale === "en" ? "bg-accent-tint text-accent" : "text-arah-muted hover:text-arah-text"}`}
              hrefLang="en"
            ...
            <Link
              ...
              className={`px-2.5 py-1.5 ${locale === "pt" ? "bg-accent-tint text-accent" : "text-arah-muted hover:text-arah-text"}`}
              hrefLang="pt"
